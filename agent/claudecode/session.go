...
type claudeSession struct {
	...
	workDir     string
	dataDir     string // data directory for attachment storage
	ctx         context.Context
...
func newClaudeSession(ctx context.Context, workDir, dataDir, model, sessionID, mode string, allowedTools []string, extraEnv []string, platformPrompt string) (*claudeSession, error) {
...
func (cs *claudeSession) Send(prompt string, images []core.ImageAttachment, files []core.FileAttachment) error {
	...
	attachDir := filepath.Join(cs.dataDir, ".cc-connect", "attachments")
