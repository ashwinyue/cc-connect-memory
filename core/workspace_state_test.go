...
func TestNormalizeWorkspacePath(t *testing.T) {
	...
	// Use normalizeWorkspacePath on the canonical path to get the expected value.
	// On macOS, /var is a symlink to /private/var, and normalizeWorkspacePath
	// strips the /private prefix for consistency.
	expectedNormalized := normalizeWorkspacePath(realDir)
	...
	tests := []struct {
		...
		{"trailing slash", realDir + "/", expectedNormalized},
		{"double slash", filepath.Join(tmp, "real-project") + "//", expectedNormalized},
		{"dot segment", filepath.Join(tmp, ".", "real-project"), expectedNormalized},
		{"dotdot segment", filepath.Join(tmp, "real-project", "subdir", ".."), expectedNormalized},
		{"symlink resolved", symlink, expectedNormalized},
