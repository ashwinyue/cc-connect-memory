...
// ReactionAdder is an optional interface for platforms that support adding
// emoji reactions to messages (e.g. Feishu, Slack). The reaction is added to
// the bot's own message to indicate completion.
type ReactionAdder interface {
	// AddDoneReaction adds a "done" reaction to the specified message.
	// messageID is the platform-specific message ID returned by Send/Reply.
	AddDoneReaction(ctx context.Context, messageID string) error
}

// BotMessageIDGetter is an optional interface for platforms that can retrieve
// the message ID of the bot's last sent message for a given session.
type BotMessageIDGetter interface {
	// GetBotMessageID returns the bot's message ID for the given session key.
	// Returns empty string if no message ID is stored.
	GetBotMessageID(sessionKey string) string
	// ClearBotMessageID clears the stored bot message ID for the given session key.
	ClearBotMessageID(sessionKey string)
}
