...
// SetMemoryProvider configures the memory system provider.
// When provider is nil, memory is disabled.
...
// SetProfileManager configures the user/group profile system.
...
func (e *Engine) processInteractiveMessageWith(p Platform, msg *Message, session *Session, agent Agent, sessions *SessionManager, interactiveKey string, workspaceDir string) {
	...
	// Identity: inject bot personality context
...
func (e *Engine) processInteractiveEvents(state *interactiveState, session *Session, sessions *SessionManager, sessionKey string, msgID string, turnStart time.Time, stopTypingFn func()) {
	...
	for {
		...
		case EventResult:
			...
			// Add DONE reaction to bot's message to indicate completion
			if idGetter, ok := p.(BotMessageIDGetter); ok {
				if reactionAdder, ok := p.(ReactionAdder); ok {
					botMsgID := idGetter.GetBotMessageID(sessionKey)
					if botMsgID != "" {
						go func(msgID string) {
							if err := reactionAdder.AddDoneReaction(e.ctx, msgID); err != nil {
								slog.Debug("failed to add DONE reaction", "error", err, "message_id", msgID)
							}
							// Clear the stored message ID after adding reaction
							idGetter.ClearBotMessageID(sessionKey)
						}(botMsgID)
					}
				}
			}
			...
			if e.memoryEnabled && e.memoryProvider != nil {
				go func() {
					...
					history := session.GetHistory(0) // 0 = get all history
					var messages []memory.MemoryMessage
					for _, h := range history {
						messages = append(messages, memory.MemoryMessage{
							Role:    h.Role,
							Content: h.Content,
						})
					}

					// Only use last few messages for extraction to avoid large payloads
					maxHistory := 10
	...
	return []helpCardGroup{
		...
		},
		{
			...
			items: []helpCardItem{
				...
				{command: "/identity", action: "cmd:/identity"},
				{command: "/profile", action: "cmd:/profile"},
