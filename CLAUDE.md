...
│  ├── identity/       │  ← bot personality (IDENTITY.md, SOUL.md)
│  ├── memory/         │  ← cross-session memory (file-based + LLM extractor)
│  └── profile/        │  ← user/group profile management (PROFILES.md)
├──────────────────────┬──────────────────────────┤
│     agent/           │      platform/           │
│  ├── claudecode/     │  ├── feishu/             │
│  ├── codex/          │  ├── telegram/           │
...
**Memory system config** (`[memory]` / per-project `[projects.*.memory]`):
```toml
[memory]
enabled   = true
...
```
{data_dir}/
  bot/{project}/        ← IDENTITY.md, SOUL.md, PROFILES.md
  memory/{project}/     ← per-project memory files
...
6. **Memory/identity enabled**: if new sub-systems added, wire `Set*` calls in `cmd/cc-connect/main.go`
