...
.PHONY: feishu-setup feishu-new feishu-bind
...
CONFIG ?= $(shell pwd)/config.toml
LOG_FILE ?= $(shell pwd)/workspace/.cc-connect/logs/cc-connect.log

install: build
	./$(APP) daemon install --config $(CONFIG) --log-file $(LOG_FILE) --work-dir $(shell pwd)

uninstall:
	./$(APP) daemon uninstall
...
# ---------------------------------------------------------------------------
# Feishu/Lark setup commands
# ---------------------------------------------------------------------------
# Usage:
#   make feishu-setup PROJECT=moon           # 扫码配置（推荐）
#   make feishu-setup PROJECT=moon APP_ID=cli_xxx:sec_xxx  # 绑定已有应用
#   make feishu-new PROJECT=moon             # 扫码创建新应用
#   make feishu-bind PROJECT=moon APP_ID=cli_xxx:sec_xxx   # 绑定已有应用
# ---------------------------------------------------------------------------
...
feishu-setup: build
ifdef PROJECT
	./$(APP) feishu setup --project $(PROJECT) $(if $(APP_ID),--app $(APP_ID),)
else
	./$(APP) feishu setup
endif
...
feishu-new: build
ifdef PROJECT
	./$(APP) feishu new --project $(PROJECT)
else
	./$(APP) feishu new
endif
...
feishu-bind: build
ifndef PROJECT
	@echo "Usage: make feishu-bind PROJECT=moon APP_ID=cli_xxx:sec_xxx"
	@exit 1
endif
ifndef APP_ID
	@echo "Usage: make feishu-bind PROJECT=moon APP_ID=cli_xxx:sec_xxx"
	...
	./$(APP) feishu bind --project $(PROJECT) --app $(APP_ID)
